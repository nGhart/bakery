...
let currentSlide = 1;
// pagination
const pageNumbersContainer = document.getElementsByClassName(
  "pageNumberContainer"
);

const nextPage = document.getElementsByClassName("nextPage");
const prevPage = document.getElementsByClassName("prevPage");

const paginationLimit = 6;
const pages = Math.ceil(menuitems.length / paginationLimit);
const currentPage = 1;
...
searchBtn.addEventListener("click", function () {
  ...
  let regency = /`${searchValue}`/gi;
  console.log(regency.test(menuitems[0].title));
  console.log(menuitems[0].title.match(regency));
...
// search
// searchBtn.addEventListener("click", function(){
//     const searchResults = menuitems.filter(function(searchkey){
//         let regency = /${searchValue}/gi
//         if(regency.test(searchkey.title) === true){
//             displayMenuItems(searchResults);
//         } else {
//             menu.innerHTML = "not found"
//         }
//     });
...
// Pagination
function getPageNumbers(page) {
  const startList = (page - 1) * paginationLimit;
  const endList = startList + paginationLimit;
  console.log(menuitems.slice(startList, endList));
}
for (let i = -1; i < 2; i++) {
